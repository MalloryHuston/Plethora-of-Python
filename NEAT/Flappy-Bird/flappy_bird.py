...
def eval_genomes(genomes, config):
    ...
    birds and sets their fitness based on the distance
    ...
    """
    1. Start by creating lists holding the genome itself,
    2. the neural network associated with the genome and,
    3. the bird object that uses that network to play
    """
    nets = []
    birds = []
    ge = []
    ...
    while run and len(birds) > 0:
        ...
        if len(birds) > 0:
            if (
                ...
                # on the screen for neural network input.
                pipe_ind = 1
        ...
        for x, bird in enumerate(
            ...
            # to determine from the neural network whether to jump or not.
            ...
            if (
                output[0] > 0.5
            ):  # We use a tanh activation function, so the result will be
                # between -1 and 1. It's over 0.5, then the bird jumps.
                bird.jump()
        ...
        if add_pipe:
            score += 1
            # Can add this line to give more reward XP
